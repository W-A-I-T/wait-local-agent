...
class AwsInventoryConnector:
    ...
    def _ec2_instance_records(
        self, session: Any, config: AwsConfig, *, strict: bool = True
    ) -> list[dict[str, Any]]:
        try:
        ...
        reservations = list(response.get("Reservations", []))
    ...
    def _s3_bucket_records(self, session: Any, *, strict: bool = True) -> list[dict[str, Any]]:
        try:
    ...
    def _security_group_records(self, session: Any, *, strict: bool = True) -> list[dict[str, Any]]:
        try:
            ec2 = self._client(session, "ec2")
        ...
        security_groups = list(response.get("SecurityGroups", []))
        next_token = response.get("NextToken")
        seen_tokens: set[str] = set()
        while next_token:
            token = str(next_token)
    ...
    def _iam_user_records(self, session: Any, *, strict: bool = True) -> list[dict[str, Any]]:
        try:
        ...
        marker = response.get("Marker") if response.get("IsTruncated") else response.get("NextToken")
        ...
        while marker:
            ...
            response = iam.list_users(Marker=token)
