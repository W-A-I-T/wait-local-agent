...
class NSightSoftwareInventoryAction:
    manifest = SmartActionManifest(
        action_id="nsight-software-inventory",
        title="N-sight software inventory",
        description=(
            "Read bounded complete software inventory for one mapped N-sight "
    ...
    def run(self, context: ActionContext, payload: dict[str, object]) -> ActionResult:
        ...
        if not isinstance(device_id, str) or not device_id.strip() or len(device_id.strip()) > 80:
            return _failed("device_id must be a non-empty string of at most 80 characters")
        provider = context.rmm_provider or LocalCollectorRmmAdapter(context.store)
        list_software = getattr(provider, "list_software", None)
        if getattr(provider, "adapter_id", "") != "n-sight" or not callable(list_software):
            return _failed("N-sight software inventory requires the N-sight RMM adapter")
        clean_device_id = device_id.strip()
        try:
            software = list_software(clean_device_id, client_id=context.client_id)
        except Exception:
            return _failed("N-sight software inventory is unavailable")
        if not isinstance(software, list) or any(not isinstance(item, dict) for item in software):
            return _failed("N-sight returned malformed software inventory data")
        output_software = [
            cast(dict[str, object], redact_value(item)) for item in software[:100]
        ]
        return ActionResult(
            status="success",
            output={
                "software": output_software,
                "count": len(output_software),
                "source": provider.adapter_id,
            },
            evidence=[
                {
                    "type": "rmm_software_inventory",
...
class NSightAntivirusUpdateHistoryAction:
    manifest = SmartActionManifest(
        action_id="nsight-antivirus-update-history",
        title="N-sight antivirus update-check history",
        description=(
            "Read bounded 60-day antivirus update-check history for one mapped "
            "N-sight server or workstation."
        ),
        kind="deterministic",
        input_schema={
    ...
    def run(self, context: ActionContext, payload: dict[str, object]) -> ActionResult:
        if set(payload) != {"device_id"}:
            return _failed("N-sight antivirus update history payload contains unsupported fields")
        device_id = payload.get("device_id")
        if not isinstance(device_id, str) or not device_id.strip() or len(device_id.strip()) > 80:
            return _failed("device_id must be a non-empty string of at most 80 characters")
        provider = context.rmm_provider or LocalCollectorRmmAdapter(context.store)
        list_history = getattr(provider, "list_antivirus_update_history", None)
        if getattr(provider, "adapter_id", "") != "n-sight" or not callable(list_history):
            return _failed(
                "N-sight antivirus update history requires the N-sight RMM adapter"
            )
        clean_device_id = device_id.strip()
        try:
            history = list_history(clean_device_id, client_id=context.client_id)
        except Exception:
            return _failed("N-sight antivirus update history is unavailable")
        if not isinstance(history, dict):
            return _failed("N-sight returned malformed antivirus update history data")
        checks = history.get("checks")
        days = history.get("days")
        if not isinstance(checks, list) or any(not isinstance(check, str) for check in checks):
            return _failed("N-sight returned malformed antivirus update-check data")
        if not isinstance(days, list) or any(
            ...
            return _failed("N-sight returned malformed antivirus update history data")
        output_days = [cast(dict[str, object], redact_value(day)) for day in days[:60]]
        output_checks = [str(redact_value(check)) for check in checks[:25]]
        return ActionResult(
            status="success",
            output={
                "checks": output_checks,
                "days": output_days,
                "count": len(output_days),
                "source": provider.adapter_id,
            },
            evidence=[
                {
                    "type": "rmm_antivirus_update_history",
                    "device_id": clean_device_id,
                    "source": provider.adapter_id,
                }
            ],
...
class NSightAntivirusScanCancelAction:
    manifest = SmartActionManifest(
        action_id="nsight-antivirus-scan-cancel",
        title="Cancel N-sight antivirus scan",
        description=(
            "Preview and, after technician approval, cancel one running "
            "managed-antivirus scan on a mapped N-sight device."
        ),
        kind="deterministic",
        input_schema={
    ...
    def run(self, context: ActionContext, payload: dict[str, object]) -> ActionResult:
        if set(payload) - {"device_id", "_approval_completed"}:
            return _failed("N-sight antivirus scan cancellation payload contains unsupported fields")
        device_id = payload.get("device_id")
        if not isinstance(device_id, str) or not device_id.strip() or len(device_id.strip()) > 80:
            return _failed("device_id must be a non-empty string of at most 80 characters")
        provider = context.rmm_provider or LocalCollectorRmmAdapter(context.store)
        cancel_scan = getattr(provider, "cancel_antivirus_scan", None)
        if getattr(provider, "adapter_id", "") != "n-sight" or not callable(cancel_scan):
            return _failed("N-sight antivirus scan cancellation requires the N-sight RMM adapter")
        clean_device_id = device_id.strip()
        if not bool(payload.get("_approval_completed")):
            return ActionResult(
                status="success",
                output={
                    "status": "preview",
                    "device_id": clean_device_id,
                    "message": "Technician approval is required before cancelling the scan.",
                    "approval_required": True,
                    "approved": False,
                },
                evidence=[
        ...
        try:
            operation = cancel_scan(clean_device_id, client_id=context.client_id)
        except Exception:
            return _failed("N-sight antivirus scan cancellation failed")
        if not isinstance(operation, dict):
            return _failed("N-sight returned malformed antivirus scan-cancellation data")
        output = cast(dict[str, object], redact_value(operation))
        output["approval_required"] = False
        output["approved"] = True
        accepted = output.get("status") == "accepted"
        return ActionResult(
            ...
            error_detail="" if accepted else "N-sight antivirus scan cancellation failed",
        )
