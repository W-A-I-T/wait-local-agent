...
- Added focused UI assertions for the active and inactive demo-mode states in the existing Settings surface test.
...
- `npm test -- --run` — passed: 69 test files, 360 tests.
- `npm test -- --run tests/wla04-surfaces.test.tsx` — passed: 1 test file, 8 tests.
- `npm test -- --run` — passed: 69 test files, 361 tests.
- `npm test -- --run` (repeat) — passed: 69 test files, 361 tests.
- `npm run build` — passed: TypeScript compilation and Vite production build (Vite 8.2.2).
- `git diff --check` — passed.
- Both test runs and the build emitted the existing Vite `configLoader: 'native'` future-warning; the build also emitted the existing large-chunk warning.

## Files Touched

- `ui/src/screens/Settings.tsx`
- `ui/tests/wla04-surfaces.test.tsx`
