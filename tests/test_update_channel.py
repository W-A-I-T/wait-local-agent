...
DOC_PUBLIC_KEY = "MSBcQFtCKN1AW2Ek6aNeeJRD9UzvlWP4gztIRRwjLD8"


def test_check_for_updates_reports_available_for_trusted_newer_release(settings) -> None:
    remote_version = _future_version()
    document = _signed_document(
        version=remote_version,
        private_key=PRIMARY_PRIVATE_KEY,
        notes_url=f"https://updates.wait.example.test/releases/{remote_version}",
        pretty=True,
    )
...
def test_second_pinned_key_verifies_rotation(settings) -> None:
    remote_version = _future_version()
    document = _signed_document(version=remote_version, private_key=SECONDARY_PRIVATE_KEY)
    active_settings = settings.__class__(
        **{
            **settings.__dict__,
            "update_channel_url": "https://updates.wait.example.test/channel.json",
...
def test_update_status_route_requires_admin_and_caches_results(settings, monkeypatch) -> None:
    ...
    viewer = resolve_auth_context(secure_settings, "Bearer viewer-token")
    technician = resolve_auth_context(secure_settings, "Bearer tech-token")
    admin = resolve_auth_context(secure_settings, "Bearer admin-token")

    assert viewer.role == Role.VIEWER
    assert technician.role == Role.TECHNICIAN
    assert admin.role == Role.ADMIN

    first = update_status_route.endpoint(admin)
    second = update_status_route.endpoint(admin)
...
def test_doc_example_matches_implementation(settings) -> None:
    ...
    assert status.status == "update_available"
    assert status.remote_version == "99.0.0"
    assert status.notes_url == "https://updates.wait.example.test/releases/99.0.0"
...
def _future_version() -> str:
    installed = parse_semver(__version__)
    return f"{installed.major + 1}.0.0"
