...
| --- | --- | --- |
| Agents vs deterministic workflows | A bounded manual agent definition can run a short, persisted sequence of existing smart actions for a ticket. It has an explicit tool allowlist, an 8-step maximum, a 120-second execution bound, tenant scope, and approval pause/resume. Event-triggered, scheduled, conversational, and unrestricted autonomous agents are not shipped. | Add event and schedule triggers only after idempotency, filters, and retry semantics are defined. |
