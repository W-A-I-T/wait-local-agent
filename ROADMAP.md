...
| --- | --- | --- |
| Agents vs deterministic workflows | A bounded agent definition can run a short, persisted sequence of existing smart actions for a ticket, manually or from a persisted five-field cron schedule. It has an explicit tool allowlist, an 8-step maximum, a 120-second execution bound, tenant scope, and approval pause/resume. Event-triggered, conversational, and unrestricted autonomous agents are not shipped. | Add event triggers, idempotency, filters, and retry semantics. |
...
| Triggered + scheduled runs | Workflow runs and persisted workflow/agent schedules are shipped through APScheduler, including pause, resume, delete, and audit tracking. | Broaden triggers, recurrence controls, windows, and safe multi-step execution. |
