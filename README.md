...
`add_note`, `update_status`, and `update_resolution` actions only when `WAIT_ALLOW_WRITE_ACTIONS=true`.
The actions use the documented `TicketNotes` and `Tickets` contracts;
`noteType`, `publish`, the status ID, and resolution text are explicit operator-supplied
