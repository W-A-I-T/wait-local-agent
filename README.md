...
- Bounded agent definitions under `/agents`, an existing-tool catalog under
  ...
  history, software/hardware inventory, Backup & Recovery session, and
...
editor paths.
The read-only `nsight-antivirus-products` action reads the bounded documented
...
The read-only `nsight-hardware-inventory` action reads the documented complete
hardware inventory for a mapped asset through `list_all_hardware`, using the
same locally verified asset ID.
...
The approval-gated `nsight-antivirus-scan-cancel` action cancels a mapped
Managed Antivirus scan through the documented `mav_scan_cancel` service using
the same approval, write-flag, and scope controls. It is reachable through the
...
window and never changes monitoring configuration. The `nsight-asset-details`
action uses the documented `list_device_asset_details` service, excludes the
provider product key, and never changes asset data. The
`nsight-monitoring-details` action uses the documented
`list_device_monitoring_details` service and never changes monitoring data. See
