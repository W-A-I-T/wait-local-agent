...
| Failure, timeout, retry, cancellation, and partial execution | AgentService, supervisor, event delivery, connector adapters | `tests/test_agents.py`, `tests/test_supervisor.py`, `tests/test_event_dispatch.py`, provider failure tests, and evaluation security-dimension tests; agent run detail includes deterministic exception/recovery, retry-parent, and partial-history evidence | Evaluation evidence is runtime/fixture evidence; it does not claim live-provider rollback unless that evidence is explicitly captured. |
