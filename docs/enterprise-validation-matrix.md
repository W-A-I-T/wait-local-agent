...
| Offline/local-first behavior | Settings offline mode, deterministic provider fallback, connector guards | `tests/test_providers.py`, `tests/test_cloud_adapters.py`, `tests/test_mcp_client.py`, and `tests/conftest.py` default settings | Optional remote providers and external connectors remain unavailable without explicit configuration and credentials. |
