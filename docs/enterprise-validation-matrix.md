...
| Prompt/tool injection and unknown operations | Evaluation contract, Work IQ policy, MCP policy, tool catalog | `tests/test_evaluation.py`, `tests/test_workiq.py`, `tests/test_mcp.py`, and tool-selection tests | Missing or unsupported security evidence fails closed. No broad mutation is enabled merely because a provider has an unverified API. |
