...
| Blueprint-to-delivery safety | Discovery, environment, architecture, governance, delivery, Power Platform stages, and YAML source packaging | `tests/test_discovery.py`, `tests/test_environment.py`, `tests/test_consultant.py`, `tests/test_delivery_plan.py`, `tests/test_power_platform_deployment.py`, `tests/test_power_platform_package.py`, and `tests/test_employee_onboarding_demo.py` | Architecture decisions consume explicit discovery read/write, approval, license, residency, data-movement, and rollback declarations as evidence-only inputs; provider permissions and authorization remain separate verification. The canonical onboarding walkthrough is `local_fixture`; it generates and validates bounded review-only manifests and a redacted non-deployable review package, plus a separate digest-bound official YAML source package. Materialization is local, workspace-confined, write-gated, and never provider deployment evidence. |
| UI route/control/state quality | React dashboard and API proxy | `ui/tests/`, `ui/package.json`, `scripts/validate_ui_browser.sh`, `docs/ui-feature-evidence.md`, `docs/ui-browser-validation.md`, and the workflow UI test/build steps | The executable real-browser matrix covers all 21 operator/direct-link destinations, named visible controls, desktop/mobile overflow, keyboard focus, controlled provider failure/recovery, controlled offline transport/recovery, and token-enforced permission state. Full per-control successful provider execution, cancellation, and live-provider behavior remain open under issue #257. |
...
On merged main commit `3fe5ec2` (PR #306), the required backend and UI CI
checks passed, and the corresponding local release gate passed:

- 2,346 backend tests passed at 95.02% coverage (95% required).
