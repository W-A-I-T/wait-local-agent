...
| Failure, timeout, retry, cancellation, partial execution, and rollback | AgentService, supervisor, event delivery, connector adapters, and the bounded Power Platform rollback primitive | `tests/test_agents.py`, `tests/test_cli.py`, `tests/test_supervisor.py`, `tests/test_event_dispatch.py`, provider failure tests, evaluation security-dimension tests, and `tests/test_power_platform_deployment.py` | Evaluation evidence is runtime/fixture evidence. The Power Platform rollback primitive verifies a prior artifact and reports PAC return status, but public approval-route/CLI wiring and live-provider rollback evidence remain open. Step retries and fallbacks remain bounded by the reviewed definition and never select an unconfigured tool. |
