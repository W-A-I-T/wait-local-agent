...
review-only OpenAPI custom-connector package and a separate Copilot Studio review handoff, prints a staged `pac solution`
...
digest. The Copilot plan is a review artifact, not a packaged component: there is
...
Copilot Studio planning is available in the Solutions Architect dashboard and
through the CLI/API. The dashboard records the returned topics, trigger
phrases, knowledge sources, and actions as a review-only plan; it does not
provision or publish the Copilot.
