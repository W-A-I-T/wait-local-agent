...
SHA-256 digest. The same evidence is assembled into a deterministic delivery
bundle containing redacted JSON files and per-file digests for operator review.
Production deployment always remains approval-required.

This is an evidence composer and handoff bundler, not an executor. The review
package and delivery bundle are not deployable Power Platform packages. The
surface does not invoke `pac`, call Microsoft services, change authorization,
create approvals, execute an agent, or deploy a package. The bundle reports
`deployable: false` and `deployment_package_generated` remains false until a
separate provider-specific packager produces and validates an official
solution artifact.
