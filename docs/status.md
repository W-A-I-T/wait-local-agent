...
- N-able N-sight tenant-scoped site, server, and workstation inventory,
  documented failing-check alerts, mapped-device outages, managed-antivirus
  threats, and Backup & Recovery session history through the XML Data Extraction
  API and an explicit client map. Patch approval is available only through the
