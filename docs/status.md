...
handoff plan and a separate deterministic, credential-free Power Platform YAML
source package bridge. It also provides a separate staged Power Platform solution
...
Studio manifests, packages their redacted metadata into a deterministic
delivery bundle with per-file digests, and separately produces a digest-bound
deployable-source package in the official YAML layout. The delivery bundle is
an operator-review handoff and explicitly reports `deployable: false`; the
separate source package means only packable local source, not provider import
or deployment.
...
- N-able N-sight tenant-scoped site, server, and workstation inventory,
  ...
  recheck; generic script catalog discovery, preview, execution, and polling
...
- ScalePad tenant-mapped Core client inventory, separately mapped ControlMap
  risk-summary and single-client compliance-health reads, and separately mapped Lifecycle Manager goal and
...
| ScalePad connector | Tenant-mapped, read-only Core client, separately mapped ControlMap risk-summary and single-client compliance-health, and separately mapped Lifecycle Manager goal/assessment reads built; documented filters and cursor pagination are bounded and surfaced through API, CLI, and Agents tooling; writes, unscoped reads, and other ScalePad product APIs remain future |
...
| RMM connectors | Local, NinjaOne, Datto, bounded N-central inventory/direct-task, N-sight device/check/check-configuration/performance-history/asset-details/monitoring-details, documented failing-check, managed-antivirus product/threat/definition/update-check-history, scan-history, and quarantine reads, approval-gated quarantine release/removal and mapped-device scan start/pause/resume/cancellation, mapped-device outage, Backup & Recovery session, and 60-day backup-check history reads, mapped-device patch inventory, approval-gated patch approval/reprocessing plus allowlisted patch policy operations, and approval-gated documented automated-task execution, Kaseya VSA X inventory/notification plus approval-gated script catalog/detail/execution/polling, and ScreenConnect session/device, approval-gated session note/message, plus optional local-command adapters built; broader vendor coverage, N-sight generic script/other antivirus mutations, backup mutations, provider-native ScreenConnect discovery/polling, and broader remediation remain future |
