...
Stored ticket, approval, audit, workflow, knowledge, and scheduled-job views
accept `client_id` where the route supports tenancy filtering. Connector
provider IDs and credentials must be configured locally; they are not accepted
in action payloads. Invalid inputs produce an explicit failed schedule/run,
not a successful-looking result.
