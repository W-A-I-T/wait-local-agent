...
| Agent definitions | SQLite definitions with enable/disable, trigger, filters, tools, steps, scope, version counter, immutable revision snapshots, restore-as-new-version, and same-tenant dependencies | `manual`, `scheduled`, and `event` triggers | Execution windows and run-to-version links | Add execution windows and link runs to revision snapshots |
