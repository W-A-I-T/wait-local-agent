...
| Agent definitions | SQLite definitions with enable/disable, trigger, filters, tools, steps, scope, version counter | `manual`, `scheduled`, and `event` triggers | Execution windows and full revision history | Add configuration revisions and restore |
...
| Versioning | Agent `version` increments on update | No historical revisions or rollback | Revision diff/restore and run-to-version link | Persist immutable agent revisions |
