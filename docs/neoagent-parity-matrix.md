...
| Tool catalog | Existing smart actions plus read-only local knowledge search exposed with schemas, risk, role, approval, and access mode | Connector/collector capabilities are not all catalog entries | Complete connector tool coverage | Wrap one existing read capability at a time |
