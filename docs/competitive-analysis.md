...
| N-able | ✓ | ✓ N-central device/issue/task metadata plus bounded approved direct-task/status path; N-sight device/check inventory, documented failing-check alerts, mapped-device patch reads, and approval-gated patch approval | Broader N-sight writes, task management, and remediation | Incremental |
