...
- N-able N-sight connector with tenant-scoped device, failing-check, managed-antivirus threat, mapped-device outage, and Backup & Recovery session reads, patch inventory, plus approval-gated patch approval/reprocessing and allowlisted policy operations
...
| N-able N-sight | Phase 4 | ✓ site/server/workstation, documented failing-check, managed-antivirus threat, mapped-device outage, and Backup & Recovery session reads, mapped-device patch inventory, and approval-gated patch approval/reprocessing plus allowlisted policy operations | Script parity, antivirus writes/scans, backup mutations, and broader writes remain unavailable | Public core |
