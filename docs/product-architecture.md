...
| N-able N-sight | Phase 4 | ✓ site/server/workstation, documented per-device check/check-configuration, performance-history, asset-details, software-inventory, and monitoring-details inventory, failing-check, managed-antivirus product/threat/definition/update-check-history, scan-history, and quarantine reads, approval-gated quarantine release/removal and mapped-device scan start/pause/resume/cancellation, mapped-device outage, Backup & Recovery session, and 60-day backup-check history reads, mapped-device patch inventory, approval-gated patch approval/reprocessing plus allowlisted policy operations, and approval-gated documented automated-task execution | Generic script catalog parity, software deployment, other antivirus mutations, backup mutations, and broader writes remain unavailable | Public core |
