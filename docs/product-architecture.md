...
- N-able N-sight connector with tenant-scoped device and per-device check/check-configuration/performance-history/asset-details/software-inventory/hardware-inventory/monitoring-details inventory, failing-check, managed-antivirus product/threat/definition/update-check-history, scan-history, and quarantine reads, approval-gated mapped-device scan start/pause/resume/cancellation, mapped-device outage, Backup & Recovery session, and 60-day backup-check history reads, patch inventory, plus approval-gated patch approval/reprocessing, allowlisted policy operations, and documented automated-task execution
...
| N-able N-sight | Phase 4 | ✓ site/server/workstation, documented per-device check/check-configuration, performance-history, asset-details, software-inventory, hardware-inventory, and monitoring-details inventory, failing-check, managed-antivirus product/threat/definition/update-check-history, scan-history, and quarantine reads, approval-gated quarantine release/removal and mapped-device scan start/pause/resume/cancellation, mapped-device outage, Backup & Recovery session, and 60-day backup-check history reads, mapped-device patch inventory, approval-gated patch approval/reprocessing plus allowlisted policy operations, and approval-gated documented automated-task execution | Generic script catalog parity, software/hardware deployment, other antivirus mutations, backup mutations, and broader writes remain unavailable | Public core |
