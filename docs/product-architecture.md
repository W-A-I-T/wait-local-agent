...
- N-able N-sight connector with tenant-scoped device and per-device check/performance-history/asset-details inventory, failing-check, managed-antivirus threat, mapped-device outage, Backup & Recovery session, and 60-day backup-check history reads, patch inventory, plus approval-gated patch approval/reprocessing and allowlisted policy operations
