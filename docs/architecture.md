...
- Agent definitions persist a short, explicit sequence of existing smart
  ...
  provider failure falls back to the reviewed sequence. Continuation decisions
  retain bounded previous-step lineage (index, tool, status, and redacted error)
  rather than persisting hidden reasoning or duplicating raw tool output.
...
- The RMM boundary normalizes tenant-scoped `endpoint-agent` collector assets
  ...
  reads documented site/server/workstation XML inventory, per-device check, performance-history, asset-details, and monitoring-details inventory, failing-check alerts,
  managed-antivirus product, threat, definition, update-check history, software/hardware inventory, and controlled scan records, mapped-device outage records, Backup & Recovery
  session records, 60-day backup-check history, and mapped-device patch records, rechecks
  ...
  and exposes bounded redacted client, risk-summary, goal, or assessment records. Core,
  ...
  device-notification reads, and approval-gated script catalog/preview,
  execution, and polling with locally persisted execution scope.
