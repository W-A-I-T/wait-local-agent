...
| End-user local ticket support | `/end-user/tickets*`, `/end-user` | Dedicated end-user surface supports token save, ticket creation, status lookup, and technician escalation; it is separate from the operator shell | Separate end-user token, fixed requester and tenant scope, end-user-safe responses, and no technician/admin tools |
