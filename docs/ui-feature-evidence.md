...
| N-sight patch operations | `/agents`, generic tool catalog/API/CLI | `nsight-patch-approve`, `nsight-patch-reprocess`, and `nsight-patch-policy` are selectable deterministic actions; policy operation is an explicit allowlist of `do_nothing`, `ignore`, `inherit`, and `retry` | All writes require technician approval, `WAIT_ALLOW_WRITE_ACTIONS=true`, mapped-device and patch-inventory rechecks, tenant scope, bounded numeric IDs, redaction, and audit evidence; no provider credentials or arbitrary service names are accepted |
