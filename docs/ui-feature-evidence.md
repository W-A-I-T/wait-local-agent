...
| N-sight check, performance, asset, monitoring, antivirus, outage, backup, patch, and automated-task operations | `/agents`, generic tool catalog/API/CLI | `nsight-check-inventory`, `nsight-check-config`, `nsight-performance-history`, `nsight-asset-details`, `nsight-monitoring-details`, `nsight-antivirus-threats`, `nsight-antivirus-scans`, `nsight-antivirus-scan-start`, `nsight-outage-lookup`, `nsight-backup-sessions`, `nsight-backup-history`, `nsight-patch-approve`, `nsight-patch-reprocess`, `nsight-patch-policy`, and `nsight-run-task-now` are selectable deterministic actions; patch, scan-start, and automated-task operations are approval-gated | Check, check-configuration, performance, asset, monitoring, antivirus threat and scan-history, outage, backup session, and backup-check history reads use a mapped-device recheck with bounded/redacted records; scan details are optional and read-only; scan start requires technician approval, `WAIT_ALLOW_WRITE_ACTIONS=true`, and a mapped-device recheck before `mav_scan_start`; asset reads omit the provider product key; patch writes require technician approval, `WAIT_ALLOW_WRITE_ACTIONS=true`, mapped-device and patch-inventory rechecks, tenant scope, bounded numeric IDs, redaction, and audit evidence; task execution requires an explicit automated-task check ID re-read from the mapped device and the same approval/write gates; no provider credentials, arbitrary service names, generic script source, or runtime script arguments are accepted |
