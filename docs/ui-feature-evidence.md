...
| Syncro, ServiceNow, Autotask, IT Glue, Confluence, Notion, SharePoint, M365, and RMM provider detail routes | API, CLI, Agents tool catalog, and Connectors dashboard panel | Connectors exposes bounded Syncro ticket-comment history, ScreenConnect approval preparation, and Notion page-comment approval preparation; other provider detail surfaces remain API/CLI/tool-catalog surfaces, with IT Glue, Notion, SharePoint, N-sight, Kaseya, and ScreenConnect device/session lookup tools selectable in `/agents` | Read-only or approval-gated provider contracts; Syncro ticket-comment history is available through the dashboard, API, CLI route, and bounded read tool, while comment writes remain approval-gated; Notion comments use explicit client/page mapping, bounded Markdown, the generic approval API, and the documented comments endpoint only after write opt-in; IT Glue and SharePoint searches are tenant/site scoped, capped at 50 results, redacted, and blocked when live probing is disabled; Notion page reads require an explicit client-page UUID map, data-source schema/rows require a separate client-data-source UUID map, and all returned data is bounded/redacted; N-sight reads require a local client map and are limited to documented site/server/workstation inventory plus derived health alerts; N-sight script/write operations remain explicit unavailable results; Kaseya VSA X reads require a local client-to-organization map and re-filter returned rows; ScreenConnect reads require a local client-to-session UUID map and documented RESTful API Manager configuration, while `screenconnect-session-note` and `screenconnect-session-message` are approval-gated through the generic API/CLI/Agents tool catalog; local catalog execution is approval-gated and accepts no runtime arguments; ScreenConnect provider-native alerts/script discovery/polling remain explicit unavailable results; ServiceNow work-note/state/assignment/resolution-metadata and Autotask ticket-note/status/resolution tools are approval-gated; disabled live calls remain reported as blocked |
