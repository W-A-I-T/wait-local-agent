...
| N-sight check, performance, asset, monitoring, antivirus, outage, backup, and patch operations | `/agents`, generic tool catalog/API/CLI | `nsight-check-inventory`, `nsight-performance-history`, `nsight-asset-details`, `nsight-monitoring-details`, `nsight-antivirus-threats`, `nsight-outage-lookup`, `nsight-backup-sessions`, `nsight-backup-history`, `nsight-patch-approve`, `nsight-patch-reprocess`, and `nsight-patch-policy` are selectable deterministic actions; policy operation is an explicit allowlist of `do_nothing`, `ignore`, `inherit`, and `retry` | Check, performance, asset, monitoring, antivirus, outage, backup session, and backup-check history reads use a mapped-device recheck with bounded/redacted records; asset reads omit the provider product key; all patch writes require technician approval, `WAIT_ALLOW_WRITE_ACTIONS=true`, mapped-device and patch-inventory rechecks, tenant scope, bounded numeric IDs, redaction, and audit evidence; no provider credentials or arbitrary service names are accepted |
