...
called out without changing the primary ownership boundary. A green integrated
PR or local release run is evidence for that slice only, not proof that every
...
| [#259](https://github.com/W-A-I-T/wait-local-agent/issues/259) | E. Evaluation / Governance | Keep open; make the enterprise-readiness matrix executable and verify backend, UI, security, and local-first gates. Each integration PR requires a fresh green backend and UI CI result after its changes. | Release-script, security-audit, and real-browser evidence remain separate gates. |
...
| [#257](https://github.com/W-A-I-T/wait-local-agent/issues/257) | F. Enterprise Readiness | Keep open; continue the route/control/state matrix, responsive behavior, and recovery pass. The current browser evidence covers all 21 operator/direct-link route headings, a mobile consultant replay, explicit offline/denied states, viewer write-control gating, and review-only artifact generation; see [browser validation](ui-browser-validation.md) and [UI/API wiring evidence](ui-feature-evidence.md). | Full control/state coverage, denied/offline/provider-error matrices, and responsive/accessibility evidence remain to be completed. |
