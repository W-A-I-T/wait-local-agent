...
| [#260](https://github.com/W-A-I-T/wait-local-agent/issues/260) | A. Core Platform | Keep open; the provider conformance matrix now indexes deterministic/local/optional-remote contracts and failure-path evidence. Live provider reachability, customer-credential validation, and any additional documented provider-specific contract remain open. | [`docs/provider-conformance-matrix.md`](provider-conformance-matrix.md), provider fixtures, and offline/remote boundary validation. |
...
| [#255](https://github.com/W-A-I-T/wait-local-agent/issues/255) | D. MSP Operations Vertical | Keep open; complete evidence-backed MSP playbooks and scheduled/event operations. | Several playbooks remain partial or fixture/import based. |
| [#254](https://github.com/W-A-I-T/wait-local-agent/issues/254) | A. Core Platform | Keep open; finish result-aware planning and exception lineage through the existing executor. | Terminal-state, retry, cancellation, and partial-history evidence must be complete. |
